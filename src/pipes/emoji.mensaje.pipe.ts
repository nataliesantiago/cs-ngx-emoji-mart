 ...
 */
@Pipe({
    name: 'replaceEmojisMensaje'
})
export class ReplaceEmojisMensajePipe implements PipeTransform {
    ...
    public static getAllTextNodes(node: Node) {
        ...
        for (node = node.firstChild; node; node = node.nextSibling) {
            if (node.nodeType === Node.TEXT_NODE) {
                ...
                all.push(...ReplaceEmojisMensajePipe.getAllTextNodes(node));
            }
        }
        return all;
    }
    ...
    public transform(
        html: string,
        set: '' | 'apple' | 'facebook' | 'twitter' | 'emojione' | 'google' | 'messenger' = ReplaceEmojisMensajePipe.DEFAULT_SHEET,
    ...
    public emojisToImages(
        ...
        const textNodes = ReplaceEmojisMensajePipe.getAllTextNodes(div);
    ...
    private get emojiRegex(): RegExp {
        if (ReplaceEmojisMensajePipe.cachedEmojiRegex) {
            return ReplaceEmojisMensajePipe.cachedEmojiRegex;
        }
        ...
        ReplaceEmojisMensajePipe.cachedEmojiRegex = new RegExp(reString, 'gu');
